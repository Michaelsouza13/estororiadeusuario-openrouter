...
const GlossaryView: React.FC = () => {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="bg-gradient-to-r from-[var(--bg-accent)] to-[#fff5f0] p-8 rounded-3xl border border-[#ff5500]/10 mb-8 flex items-start gap-4 shadow-sm">
        <div className="bg-[#ff5500] p-3 rounded-full text-white shadow-lg shadow-[#ff5500]/20">
            <BookMarked size={24} />
        </div>
        <div className="flex-1">
            <h3 className="text-xl font-black text-[var(--text-primary)] mb-2">Glossário de Termos</h3>
            ...
            <div className="mt-4 flex items-center gap-2 text-xs font-bold text-[#ff5500] bg-[var(--bg-surface)]/60 px-3 py-2 rounded-lg inline-flex border border-[#ff5500]/10">
                <Info size={14} />
        ...
        <div className="text-right">
          ...
          <div className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest">termos</div>
        </div>
      </div>

      <div className="bg-[var(--bg-surface)] rounded-[32px] shadow-[var(--shadow-surface)] border border-[var(--border-light)] overflow-hidden">
        <div className="p-4 border-b border-[var(--border-muted)] flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2.5 border border-[var(--border-light)] rounded-xl focus:ring-2 focus:ring-[#ff5500] outline-none w-full font-bold text-[var(--text-primary)] bg-[var(--bg-surface)] placeholder:text-[var(--text-muted)]"
              value={searchTerm}
        ...
        {showForm && (
          <div className="p-4 bg-[var(--bg-accent)] border-b border-[#ff5500]/10 animate-in fade-in slide-in-from-top-2">
            <div className="flex flex-col md:flex-row gap-3 items-end">
              <div className="flex-1 w-full">
                <label className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest mb-1 block">Termo / Sigla</label>
                <input
                  autoFocus
                  className="w-full border border-[var(--border-light)] rounded-xl px-4 py-2.5 font-bold text-[var(--text-primary)] bg-[var(--bg-surface)] outline-none focus:border-[#ff5500] transition-all uppercase"
                  placeholder="Ex: GCJ"
                  value={formTerm}
              ...
              <div className="flex-[2] w-full">
                <label className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest mb-1 block">Significado</label>
                <input
                  className="w-full border border-[var(--border-light)] rounded-xl px-4 py-2.5 font-bold text-[var(--text-primary)] bg-[var(--bg-surface)] outline-none focus:border-[#ff5500] transition-all"
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setShowForm(false); setFormTerm(''); setFormMeaning(''); }} className="bg-[var(--bg-muted)] text-[var(--text-secondary)] p-2.5 rounded-xl hover:bg-[var(--border-light)] transition-all"><X size={18} /></button>
              </div>
            </div>
            </div>
          )}
        ...
        {filtered.length === 0 ? (
          <div className="p-16 text-center">
            <BookMarked className="mx-auto text-[var(--text-muted)] mb-4" size={48} />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-[var(--border-muted)]">
                {filtered.map((entry) => (
                  <tr key={entry.term} className="hover:bg-[var(--hover-bg)] transition-colors group">
                    <td className="px-6 py-4">
                      <span className="font-black text-[#ff5500] bg-[var(--bg-accent)] px-3 py-1 rounded-lg text-xs inline-block border border-[#ff5500]/10">
                        {entry.term}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      {editingTerm === entry.term ? (
                        <div className="flex items-center gap-2">
                          <input
                            autoFocus
                            className="flex-1 border border-[#ff5500] rounded-lg px-3 py-1.5 text-sm font-bold text-[var(--text-primary)] bg-[var(--bg-surface)] outline-none"
                            value={editMeaning}
                          ...
                          <button onClick={() => setEditingTerm(null)} className="text-[var(--text-secondary)] p-1"><X size={16} /></button>
                        </div>
                      ) : (
                        <span className="font-bold text-[var(--text-primary)]">{entry.meaning}</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="p-1.5 text-[var(--text-muted)] hover:text-[#ff5500] hover:bg-[var(--hover-bg)] rounded-lg transition-all"
                        >
                          <Pencil size={14} />
                        </button>
                        {deleteConfirm === entry.term ? (
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => setDeleteConfirm(null)} className="p-1.5 text-[var(--text-muted)] hover:bg-[var(--bg-muted)] rounded-lg transition-all"><X size={14} /></button>
                          </div>
                        ) : (
                          <button
